...
echo "${CYAN_TEXT}${BOLD_TEXT}      EDULINKUP CLOUD LAB AUTOMATION        ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD_TEXT}============================================${RESET_FORMAT}"
echo
echo "${GREEN_TEXT}${BOLD_TEXT}Subscribe to EduLinkUp for more cloud labs:${RESET_FORMAT}"
...
# Create cluster
echo "${CYAN_TEXT}${BOLD_TEXT}Creating GKE cluster 'my-cluster-1'...${RESET_FORMAT}"
...
echo "${MAGENTA_TEXT}${BOLD_TEXT}Configuring Binary Authorization policy...${RESET_FORMAT}"
...
echo "${YELLOW_TEXT}${BOLD_TEXT}Attempting to create pod with denied image...${RESET_FORMAT}"
cat << EOF | kubectl create -f -
apiVersion: v1
kind: Pod
metadata:
...
echo -e "${YELLOW_TEXT}${BOLD_TEXT}Edit Binary Policy at: ${BLUE_TEXT}${UNDERLINE_TEXT}https://console.cloud.google.com/security/binary-authorization/policy?project=$DEVSHELL_PROJECT_ID${RESET_FORMAT}"
echo
...
echo
echo "${CYAN_TEXT}${BOLD_TEXT}============================================${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD_TEXT}   LAB COMPLETED SUCCESSFULLY WITH EDULINKUP ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD_TEXT}============================================${RESET_FORMAT}"
echo
echo "${GREEN_TEXT}${BOLD_TEXT}Thank you for learning with EduLinkUp!${RESET_FORMAT}"
echo "${BLUE_TEXT}${BOLD_TEXT}For more tutorials, subscribe to:${RESET_FORMAT}"
echo "${BLUE_TEXT}${BOLD_TEXT}${UNDERLINE_TEXT}https://www.youtube.com/@EduLinkUp${RESET_FORMAT}"
echo
