...
echo "${CYAN_TEXT}${BOLD_TEXT}Step 3:${RESET_FORMAT} ${WHITE_TEXT}Creating a new Compute Engine instance named 'edulinkup'.${RESET_FORMAT}"
gcloud compute instances create edulinkup --project=$DEVSHELL_PROJECT_ID --zone $ZONE --machine-type=e2-medium --create-disk=auto-delete=yes,boot=yes,device-name=edulinkup,image=projects/windows-cloud/global/images/windows-server-2022-dc-v20230913,mode=rw,size=50,type=projects/$DEVSHELL_PROJECT_ID/zones/$ZONE/diskTypes/pd-balanced 
...
gcloud compute instances get-serial-port-output edulinkup --zone=$ZONE
...
gcloud compute reset-windows-password edulinkup --zone $ZONE --user admin --quiet
