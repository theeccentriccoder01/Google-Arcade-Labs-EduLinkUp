...
gcloud compute networks subnets create custom-subnet-us --network=custom-vpc --region=[replace with your region 1] --range=10.0.1.0/24 && \
gcloud compute networks subnets create custom-subnet-asia --network=custom-vpc --region=[replace with your region 2] --range=10.0.2.0/24
```

> **⚠️ Region Note:** Replace `[replace with your region 1]` and `[replace with your region 2]` with valid region values, for example `us-central1` and `asia-south1`.
