...
[![Open Lab](https://img.shields.io/badge/▶️_Open_Lab-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white)](https://www.skills.google/games/7008/labs/43562)
...
export ZONE=$(gcloud compute instances list --limit=1 --format="value(zone)")
export VM_NAME=$(gcloud compute instances list --limit=1 --format="value(name)")
export BASE64_KEY=$(head -c 32 /dev/urandom | base64)

gcloud compute disks create csek-encrypted-disk \
  --size=200GB \
  --zone="$ZONE" \
  --csek-key-file=<(cat <<EOF
[{
  "uri": "https://www.googleapis.com/compute/v1/projects/$PROJECT_ID/zones/$ZONE/disks/csek-encrypted-disk",
  "key": "$BASE64_KEY",
  "key-type": "raw"
}]
EOF
